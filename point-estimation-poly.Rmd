...
## Feature expansion 2: polynomial expansion

```{R}
# Prepare for expanding features
ids <- select(nsrdb, ID)
response <- select(nsrdb, GHI)
predictors <- select(nsrdb, -GHI, -ID)

# Generate squared terms
clouds <- select(nsrdb, starts_with("Cloud"))  # Temporarily remove b/c can't square factors
sq_names <- str_c(names(select(predictors, -starts_with("Cloud"))), "_sq")
predictors_sq <- select(predictors, -starts_with("Cloud"))^2
colnames(predictors_sq) <- sq_names

# Generate pair-wise interaction terms
predictors <- as_tibble(model.matrix(~ .^2, predictors))

# Merge expanded features into original tibble
nsrdb <- cbind(ids, predictors, predictors_sq, response)
rm(ids, clouds, predictors, predictors_sq, response)
```
...
```{R}
solar_predict <- predict(model, valid_data)
...
```{R}
solar_predict <- predict(model, test_data)
